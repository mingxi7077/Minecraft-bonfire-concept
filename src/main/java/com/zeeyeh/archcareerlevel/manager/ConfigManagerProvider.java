...
public class ConfigManagerProvider {
    ...
    private void initConfigs() {
        File[] files = ArchCareerLevel.getInstance().getDataFolder().listFiles(file -> file.getName().endsWith(".yml") && file.isFile());
        if (files == null) {
            return;
        }
        for (File file : files) {
            ...
            MessageUtil.sendMessage(Bukkit.getConsoleSender(), ArchCareerLevelLangApi.translate("loadConfig")
