...
public class LanguageManager {
    ...
    public LanguageManager() {
        try {
            ...
            languageFolder = new File(InfiniteMining.getInstance().getDataFolder(), "locales");
            if (!languageFolder.exists()) {
                ClassLoader classLoader = InfiniteMining.class.getClassLoader();
                ...
                while (entries.hasMoreElements()) {
                    ...
                    if (name.startsWith("locales/") && !jarEntry.isDirectory()) {
                        IoUtil.copy(classLoader.getResourceAsStream(name), Files.newOutputStream(new File(InfiniteMining.getInstance().getDataFolder(), name).toPath()));
                    }
                }
            }
        } catch (Exception e) {
            ...
            Bukkit.getPluginManager().disablePlugin(InfiniteMining.getInstance());
        }
        initLangs();
    }
    ...
    public void initLangs() {
        ...
        langName = InfiniteMining.getInstance().getConfig().getString("locale");
    }
    ...
    public Object translate(String path) {
        if (langName == null) {
            Bukkit.getConsoleSender().sendMessage("Plugin language configuration error, please go to the " +
                    new File(InfiniteMining.getInstance().getDataFolder(), "config.yml").getAbsolutePath() +
