...
public class ConfigManagerProvider {
    ...
    private void initConfigs() {
        File[] files = MultiCurrency.getInstance().getDataFolder().listFiles(file -> file.getName().endsWith(".yml") && file.isFile());
        if (files == null) {
            return;
        }
        for (File file : files) {
    ...
    public boolean registerConfig(String name) {
        ...
        File file = new File(MultiCurrency.getInstance().getDataFolder(), name + ".yml");
        if (!file.exists()) {
            return false;
        }
        YamlConfiguration configuration = YamlConfiguration.loadConfiguration(file);
